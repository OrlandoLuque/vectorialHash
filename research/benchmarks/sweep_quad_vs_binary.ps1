# Quadtree vs binary tree at the recommended item_limit. The whole tuning
# story (il=100 optimum) was derived on the binary tree; BENCHMARKS
# Results 6 found the quadtree ahead at il=3/6. Does that hold at il=100?
#
# Runs mode=binary and mode=quad separately (clean per-structure timing,
# no shared-process cache interaction). pop=10000, il ∈ {30,100,200},
# lca, full sim, --respawn 0.05, 3 seeds, neighbors off.

param(
    [string]$CsvPath = "docs/sweep_quad_vs_binary.csv",
...
Import-Csv $CsvPath | ForEach-Object { [void]$done.Add("$($_.pop)|$($_.item_limit)|$($_.mode)|$($_.seed)") }

# Parser takes the struct key (binary or quad) so we read the right rows.
function Get-NumberAfter([string]$lines, [string]$structKey, [string]$label) {
...
$totalCells = $itemLimits.Count * $modes.Count * $seeds.Count
$cellIdx = 0
$swStart = Get-Date

foreach ($il in $itemLimits) {
    foreach ($mode in $modes) {
        $structKey = $mode  # output rows are labelled "binary" / "quad"
        $leafPat = if ($mode -eq 'binary') { 'binary:\s+(\d+)\s+leaves' } else { 'quad:\s+(\d+)\s+leaves' }
        foreach ($seed in $seeds) {
            $cellIdx++
            $key = "$pop|$il|$mode|$seed"
            ...
            $cellSw = Get-Date
            $stdout = & $BinPath --mode $mode --frames $frames --warmup $warmup `
            ...
            $mv  = Get-NumberAfter $stdout $structKey 'move+update (us)'
            $vis = Get-NumberAfter $stdout $structKey 'vision cull avg (us)'
            $atk = Get-NumberAfter $stdout $structKey 'attack cull avg (us)'
            $rm  = Get-NumberAfter $stdout $structKey 'insert+remove (us)'
            $alive  = Get-IntAfter $stdout 'alive\s+(\d+)'
            $leaves = Get-IntAfter $stdout $leafPat
            ...
            [void]$done.Add($key)
            Write-Host ("  -> mv={0:N0} vis={1:N1} atk={2:N1} | leaves={3} | {4:N1}s" -f $mv[0], $(if ($vis) { $vis[0] } else { 0 }), $(if ($atk) { $atk[0] } else { 0 }), $leaves, $cellSecs)
        }
    }
}
